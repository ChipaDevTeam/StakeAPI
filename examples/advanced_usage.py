...
class StakeAnalytics:
    ...
    async def analyze_games_by_provider(self) -> Dict[str, Dict[str, Any]]:
        ...
        async with StakeAPI(api_key=self.api_key) as client:
            ...
            for provider, stats in provider_stats.items():
                ...
                stats["min_bet_avg"] = sum(stats["min_bet_range"]) / len(
                    stats["min_bet_range"]
                )
                stats["max_bet_avg"] = sum(stats["max_bet_range"]) / len(
                    stats["max_bet_range"]
                )
    ...
    async def get_user_performance_stats(self) -> Dict[str, Any]:
        ...
        async with StakeAPI(api_key=self.api_key) as client:
            ...
            roi = (
                ((total_won - total_wagered) / total_wagered * 100)
                if total_wagered > 0
                else 0
            )
            ...
            for bet in bets:
                ...
                if game_id:
                    ...
                    if bet.status == "won":
                        game_profits[game_id]["profit"] += float(
                            bet.potential_payout - bet.amount
                        )
            ...
            most_profitable = (
                max(game_profits.items(), key=lambda x: x[1]["profit"])
                if game_profits
                else None
            )
            ...
            return {
                ...
                "most_profitable_profit": (
                    most_profitable[1]["profit"] if most_profitable else 0
                ),
            }
...
async def batch_game_analysis():
    ...
    for provider, stats in sorted(
        provider_stats.items(), key=lambda x: x[1]["game_count"], reverse=True
    )[:10]:
        print(f"\n{provider}:")
